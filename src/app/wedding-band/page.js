import { WeddingBands } from "./weddingBands/WeddingBands";

const fetchMeta = async () => {
  let gemstone = [];
  try {
    const response = await fetch(
      `${process.env.BASE_URL}/check?menu=wedding-band`
    );
    if (!response.ok) {
...
export async function generateMetadata() {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url:  "https://assets.rocksama.com/frontend/images/hand3.jpg",
          width: 800,
          height: 600,
...
const weddingBand = async ({searchParams, params}) => {
  ...
  return (
    <>
      <WeddingBands gemstoneData={Metadata} />
    </>
  );
};
