...
const SearchPage = ({
  ...
  if (typeof window !== "undefined") {
    $(document).ready(function () {
      $(".resultdata > div.all-pages-data").each(function (i, odiv) {
        $(".dropdown-content .ss__list > .filter-ss-column > input").on(
          "click",
          function () {
  ...
  useEffect(() => {
    const debounceUpdatesecureLocalStorage = debounce(() => {
      Cookies.set("searchMetal", JSON.stringify(selectedMetalIds), {
        expires: 3650,
        secure: true,
        sameSite: "Strict",
      });
      Cookies.set("searchStyle", JSON.stringify(selectedStyles), {
        expires: 3650,
        secure: true,
        sameSite: "Strict",
      });
      Cookies.set("searchShape", JSON.stringify(shapeFilter), {
        expires: 3650,
        secure: true,
        sameSite: "Strict",
      });
    ...
    <>
      ...
      <div className="search-filter-sort-parent">
        <div className="searching-bar-field container">
          ...
          <button type="search" onClick={() => handleSearchQuery()}>
            search
          </button>
        </div>
      </div>
          ...
          <div className="bredCramStyleFilter">
            ...
            {metalColorName && (
              <div className="breadCram">
                <Link
                  href="#"
                  onClick={() => handleRemoveColor(metalColorName)}
                >
                  {metalColorName}{" "}
                  <span>
                    <IoIosClose />
