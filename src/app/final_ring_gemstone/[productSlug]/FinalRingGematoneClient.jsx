...
export default function FinalGemstone({ gemstoneDataServer, ringData }) {
  ...
  const handleCreateAccount = async (
    ...
    setFormData(a);
    var newAPI = `${baseUrl}/cart?user_id=${formData.user_id}&ring_price=${
      formData.ring_price
    }&ring_id=${formData.ring_id}&ring_color=${
      formData.ring_color
    }&gemstone_id=${formData.gemstone_id}&gemstone_stock_no=${
      formData.gemstone_stock_num
    }&gemstone_price=${formData.gemstone_price}&img_sku=${
      formData.img_sku
    }&product_type=${formData.product_type}&ring_size=${
      formData.ring_size
    }&ring_type=${diamond_original}${
  ...
  const handleWishlist = async (
    ...
    const gem_URL = `${baseUrl}/add_to_wishlist?user_id=${
      formData.user_id
    }&ring_price=${formData.ring_price}&ring_id=${
      formData.ring_id
    }&ring_color=${formData.ring_color}&product_type=${
      formData.product_type
    }&img_sku=${formData.img_sku}&gemstone_price=${
      formData.gemstone_price
    }&gemstone_id=${
      formData.gemstone_id
    }&ring_type=${diamond_original}&ring_size=${
      formData.ring_size
    }&gemstone_stock_no=${formData.gemstone_stock_num}${
      textEngraving ? `&engraving=${textEngraving}` : ""
    }${font_style ? `&font=${font_style}` : ""}`;
              ...
              <>
                ...
                <div className="main-arrow-heading" key={diamondItem.id}>
                  <Tabbing
                    ...
                    gemStoneLink={"/engagement-rings/start-with-a-gemstone"}
                    type="gem-final"
                  />
                </div>
